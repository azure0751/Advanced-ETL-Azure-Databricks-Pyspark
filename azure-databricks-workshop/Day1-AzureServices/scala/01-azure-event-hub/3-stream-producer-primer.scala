...
// MAGIC %md **Note:**connection string should look like "Endpoint=sb://SAMPLE;SharedAccessKeyName=KEY_NAME;SharedAccessKey=KEY;EntityPath=EVENTHUB_NAME"
// MAGIC
// MAGIC Resize cluster to a larger one if running on single worker.
...
val connectionString = ConnectionStringBuilder(dbutils.secrets.get(scope = "gws-crimes-aeh", key = "conexion-string"))
  .setEventHubName("wrkshpeventhub")
  .build
...
// MAGIC %fs ls /mnt/workshop/curated/crimes/chicago-crimes
...
val producerDF = sourceDF
                    .where("case_year > 2001")
                    .select($"case_id" as "partitionKey", (to_json(struct(
...
// 6) Stream crime events to event hub
