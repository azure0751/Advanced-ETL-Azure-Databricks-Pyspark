...
// MAGIC %sql
// MAGIC select count(*) from CRIMES_DB.CHICAGO_CRIMES_CURATED WHERE case_year = 2001 AND case_month > 8;

// COMMAND ----------

// MAGIC %sql
// MAGIC select count(*) from CRIMES_DB.CHICAGO_CRIMES_CURATED LIMIT 10;
...
spark.table("crimes_db.chicago_crimes_curated").where("case_year = 2001 and case_month = 7")
     .write
...
val df = spark.sql("select * from CRIMES_DB.CHICAGO_CRIMES_CURATED").where("case_year = 2001 and case_month > 8")
