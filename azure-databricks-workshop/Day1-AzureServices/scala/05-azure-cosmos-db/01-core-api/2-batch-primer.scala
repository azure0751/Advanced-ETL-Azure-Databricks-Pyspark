...
// MAGIC https://search.maven.org/search?q=a:azure-cosmosdb-spark_2.4.0_2.11
...
val df = spark.sql("select case_id, primary_type as case_type, case_year, case_month, case_day_of_month from crimes_db.chicago_crimes_curated where case_year=2018 and case_month = 7")
display(df)
