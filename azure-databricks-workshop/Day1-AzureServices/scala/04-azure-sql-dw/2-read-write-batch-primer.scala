...
// DBTITLE 0,1. Review the source dataset
// MAGIC %md
...
//replace with your blob storage account name
spark.conf.set(
  "fs.azure.account.key.sreeramwrkshpdb.blob.core.windows.net",
...
// Verify that the data source class for the connector is present in your cluster’s class path
...
val df = spark.sql("select case_id,primary_type,cast(arrest_made as string),case_year,case_month,case_day_of_month from crimes_db.chicago_crimes_curated where case_year = 2018 and case_month = 7")
display(df)
...
df.write
  ...
  .option("tempDir", "wasbs://scratch@sreeramwrkshpdb.blob.core.windows.net/sqldwbatch-tempdir")
  .save()
...
//dont forget to replace with your blob storage account name
df.write
  .format("com.databricks.spark.sqldw")
